...
export class PostController {
    ...
    async create(req: Request, res: Response, next : NextFunction){
        try {
            const result = await this.createPostUseCase.execute({authorId : req.userId ,...req.body} as ICreatePostRequestDTO)
    ...
    async delete(req: Request, res: Response, next : NextFunction){
        try {
            const result = await this.deletePostUseCase.execute({...req.body} as IDeletePostRequestDTO)
            res.status(201).json({result})
        } catch (error) {
            next(error)
        }
    ...
    async getPost(req: Request, res: Response, next : NextFunction){
        try {
            const result = await this.getPostUseCase.execute({...req.body} as IGetPostRequestDTO)
     ...
     async listPost(req: Request, res: Response, next : NextFunction){
        try {
            const result = await this.listPostUseCase.execute({...req.body} as IListPostRequestDTO)
            res.status(201).json({result})
        } catch (error) {
            next(error)
        }
